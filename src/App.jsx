...
function App() {
    ...
    const checkAuth = async()=>{
      ...
      if(token){
        ...
        if(res){
          ...
          return context.USER_DISPATCHER({type:USER.LOGGED_IN ,payload:res})
  ...
  return (
    <div className="relative w-full h-full flex flex-col bg-[#fffffe] backdrop-blur-lg items-center dark:bg-[#16161a] dark:text-white duration-500">
      <Navbar />
      ...
      {
        pathName === '/' ? 
        <img src={bg2} alt="bg_blur" className="absolute w-full h-full object-cover top-[-15%] -z-10  "/>
        :
